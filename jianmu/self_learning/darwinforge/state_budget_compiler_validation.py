...
def run_state_budget_compiler_validation(
    ...
    trace_manifest = {"trace_sharded": True, "shards": [], "per_profile": {}}
    for profile_name, samples in profile_samples.items():
        profile_dir = out / "compiler_validation" / profile_name
        metrics = run_targeted_compiler_validation(profile_dir, samples["supported"][:2000], samples["boundary"][:2000], compile_worker_count=compile_worker_count)
        ...
        trace_manifest["shards"].append({"profile_name": profile_name, "path": shard_name, "row_count": sum(1 for _ in shard_dst.open(encoding="utf-8"))})
        trace_manifest["per_profile"][profile_name] = metrics
    result = {
    ...
    (out / "compiler_validation_trace_manifest.json").write_text(json.dumps(trace_manifest, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return result
