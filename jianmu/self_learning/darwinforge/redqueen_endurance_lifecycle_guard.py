...
def run_endurance_lifecycle_guard(repo_root: str | Path, output_records: str | Path, cycle_result: Dict[str, Any], idle_grace_seconds: int = 30) -> Dict[str, Any]:
    ...
    result = {
        "endurance_lifecycle_guard_completed": True,
    ...
    result["lifecycle_clean_after_endurance"] = all([
    ...
    result["endurance_lifecycle_guard_passed"] = result["lifecycle_clean_after_endurance"] and all(c.get("cycle_lifecycle_checkpoint_passed", False) for c in checkpoints)
    _write_json(Path(output_records) / "endurance_lifecycle_guard.json", result)
    return result
