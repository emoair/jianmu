...
def audit_dry_run_compiler_accounting(output_records: str | Path, rows: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    rows = list(rows)
    ...
    (out / "dry_run_compiler_accounting.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return result


def detect_stubbed_validation(rows: Iterable[Dict[str, Any]]) -> bool:
    return any(row.get("stubbed") or (row.get("passed") and not row.get("exe_run")) for row in rows)
