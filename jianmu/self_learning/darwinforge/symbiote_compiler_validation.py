...
def run_symbiote_compiler_validation(output_records: str | Path, target: int = 5000, compile_worker_count: int = 16) -> Dict[str, Any]:
    out = Path(output_records)
    base = run_codecartographer_compiler_validation(out, target=target, compile_worker_count=compile_worker_count)
    result = {
    ...
    _write_json(out / "symbiote_compiler_validation.json", result)
    manifest = out / "codecartographer_compiler_trace_manifest.json"
    if manifest.exists():
        shutil.copyfile(manifest, out / "symbiote_compiler_trace_manifest.json")
    return result
