...
def run_redqueen_hydrabudget_compiler_validation(redqueen_dataset_dir: str | Path, output_records: str | Path, supported_spot: int = 500, boundary_spot: int = 500, compile_worker_count: int = 16) -> Dict[str, Any]:
    out = Path(output_records)
    supported = _collect_supported(Path(redqueen_dataset_dir), supported_spot)
    ...
    for i in range(boundary_spot):
        trace.append({"sample_id_hash": _hash(f"boundary:{i}"), "support_status": "boundary", "compiler_invoked": False, "compiler_verified_correct": False})
...
def _collect_supported(root: Path, limit: int) -> List[Dict[str, Any]]:
    rows = []
...
def _validate(row: Dict[str, Any], backend: Any) -> Dict[str, Any]:
    base = {"sample_id_hash": _hash(row["id"]), "support_status": row["support_status"], "compiler_invoked": False, "compiler_verified_correct": False}
